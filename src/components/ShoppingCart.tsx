import type {ShopCartProdType, TableShopCartDataType} from "../utils/shop-types.ts";
...
const ShoppingCart = () => {
    ...
    const getTableShopCartProduct = (prod: ShopCartProdType) => {
        ...
        let res:TableShopCartDataType = {id: undefined, amount: 0, category: "", price: 0, count: 0, img: "", title: "", unit: ""}
        if (!product) {
            console.log(authUser!.email);
            removeProductFromCart(`${authUser!.email}_collection`, prod.cartProdId)
        }
        else res  = {...product, count: prod.count, amount: prod.count * product.price};
        return res;
    }
